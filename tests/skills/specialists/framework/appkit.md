# appkit behavioral fixture

## Material-gap case

Review a plan in this domain whose available evidence does not establish the
relevant Application and window lifecycle, responder chain, controllers,
...
Review the same plan after evidence establishes the target AppKit and macOS
...
- Domain boundary: macOS range, architectures,
  sandbox, entitlements, helpers, document and file access, signing,
  notarization, packaging, update, and distribution.
...
### `references/documents-and-state-restoration.md`

Positive trigger: the plan changes document architecture, opening, autosave, undo, conflict handling, coordinated access, state restoration, close behavior, or document recovery.

Expected behavior:

- Standard or deep mode loads `references/documents-and-state-restoration.md`.
- The review applies the focused checks in `references/documents-and-state-restoration.md`.

Negative trigger: Review the same specialist with no affected boundary named in the positive trigger.

Expected behavior:

- Standard or deep mode does not load `references/documents-and-state-restoration.md`.
- Missing evidence follows the material-question or uncertainty policy instead of loading every focused reference.

### `references/sandbox-entitlements-and-file-access.md`

Positive trigger: the plan changes sandboxing, entitlements, privacy permissions, security-scoped URLs, helper access, filesystem access, automation, hardware access, or denied permission behavior.

Expected behavior:

- Standard or deep mode loads `references/sandbox-entitlements-and-file-access.md`.
- The review applies the focused checks in `references/sandbox-entitlements-and-file-access.md`.

Negative trigger: Review the same specialist with no affected boundary named in the positive trigger.

Expected behavior:

- Standard or deep mode does not load `references/sandbox-entitlements-and-file-access.md`.
- Missing evidence follows the material-question or uncertainty policy instead of loading every focused reference.
