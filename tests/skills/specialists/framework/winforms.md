# winforms behavioral fixture

## Material-gap case

Review a plan in this domain whose available evidence does not establish the
relevant Control and window lifetime, UI thread, message loop, events, data
binding, scaling, resources, async work, interop, accessibility, configuration,
and deployment.
...
Review the same plan after evidence establishes the target .NET and WinForms
versions, Windows targets, architecture, DPI and localization targets,
packaging, deployment, native interop, and support constraints, plus the
...
- Domain boundary: Windows and .NET versions,
  architecture, desktop session, native interop, DPI, installer, signing,
  update, configuration, file access, and support channel.
...
Positive trigger: the plan changes P/Invoke, COM, ActiveX, handles, native ownership, architecture, string encoding, registration, application settings migration, packaging, signing, install scope, repair, update, rollback, or uninstall behavior.
