# wpf behavioral fixture

## Material-gap case

Review a plan in this domain whose available evidence does not establish the
relevant Application and dispatcher lifecycle, dependency properties, binding,
resources, templates, commands, threading, async behavior, rendering, DPI,
accessibility, interop, and deployment.
...
Review the same plan after evidence establishes the target .NET and WPF
versions, Windows targets, architecture, packaging, deployment, DPI and
localization targets, interop, and application lifecycle, plus the
...
- Domain boundary: Windows and .NET versions,
  architecture, desktop session, native interop, rendering mode, installer,
  signing, configuration, update, and file access.
...
### `references/native-interop-packaging-and-updates.md`

Positive trigger: the plan changes P/Invoke, COM, HWND hosting, native callbacks, architecture, trimming, deployment mode, runtime dependencies, signing, installers, settings migration, update, rollback, repair, or uninstall behavior.

Expected behavior:

- Standard or deep mode loads `references/native-interop-packaging-and-updates.md`.
- The review applies the focused checks in `references/native-interop-packaging-and-updates.md`.

Negative trigger: Review the same specialist with no affected boundary named in the positive trigger.

Expected behavior:

- Standard or deep mode does not load `references/native-interop-packaging-and-updates.md`.
