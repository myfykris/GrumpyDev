# graphql-api-design behavioral fixture
...
- Domain boundary: Gateway and service topology,
  resolver placement, batching scope, cache, subscription transport, proxy
  limits, authorization boundary, scaling, and persisted-operation deployment.
...
### `references/subscriptions-and-persisted-operations.md`

Positive trigger: the plan changes subscriptions, long-lived authorization, token expiry, revocation, reconnect, fan-out, event filtering, persisted operations, operation registration, allowlists, rollout, or revocation.

Expected behavior:

- Standard or deep mode loads `references/subscriptions-and-persisted-operations.md`.
- The review applies the focused checks in `references/subscriptions-and-persisted-operations.md`.

Negative trigger: Review the same specialist with no affected boundary named in the positive trigger.

Expected behavior:

- Standard or deep mode does not load `references/subscriptions-and-persisted-operations.md`.
