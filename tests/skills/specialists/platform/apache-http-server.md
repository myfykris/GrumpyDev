# apache-http-server behavioral fixture

## Material-gap case

Review a plan in this domain whose available evidence does not establish the
relevant MPM choice, request routing, virtual hosts, modules, reverse proxying,
TLS, authentication, authorization, headers, rewrites, PHP integration,
timeouts, limits, logs, reload, and rollback.
...
Review the same plan after evidence establishes the target Apache version, MPM,
enabled modules, virtual hosts, TLS termination, proxy topology, PHP SAPI, user
and privilege model, configuration ownership, and reload process, plus the
...
- Domain boundary: version,
  distribution, MPM, modules, virtual hosts, listeners, TLS, proxying, PHP
  SAPI, privileges, limits, reload, and rollback coverage.
...
### `references/routing-proxy-and-php-integration.md`

Positive trigger: the plan changes listeners, virtual hosts, default hosts, document roots, configuration merge order, htaccess, rewrites, proxying, forwarded headers, CGI, handlers, mod_php, FastCGI, PHP-FPM, script mapping, path info, or upstream timeouts.

Expected behavior:

- Standard or deep mode loads `references/routing-proxy-and-php-integration.md`.
- The review applies the focused checks in `references/routing-proxy-and-php-integration.md`.

Negative trigger: Review the same specialist with no affected boundary named in the positive trigger.

Expected behavior:

- Standard or deep mode does not load `references/routing-proxy-and-php-integration.md`.
- Missing evidence follows the material-question or uncertainty policy instead of loading every focused reference.

### `references/tls-access-control-and-reload.md`

Positive trigger: the plan changes TLS, certificates, client authentication, HSTS, authentication, authorization, filesystem access, module privileges, status endpoints, graceful reload, old workers, logs, or configuration rollback.

Expected behavior:

- Standard or deep mode loads `references/tls-access-control-and-reload.md`.
- The review applies the focused checks in `references/tls-access-control-and-reload.md`.

Negative trigger: Review the same specialist with no affected boundary named in the positive trigger.

Expected behavior:

- Standard or deep mode does not load `references/tls-access-control-and-reload.md`.
- Missing evidence follows the material-question or uncertainty policy instead of loading every focused reference.
