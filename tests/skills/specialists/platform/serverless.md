...
- What delivery, concurrency, timeout, retry, state, networking, permission,
  and cost behavior follows from that platform?
...
- Domain boundary: provider, region,
  runtime, trigger, concurrency, timeout, ephemeral storage, identity,
  networking, cold start, retries, destinations, limits, and deployment
  coverage.
